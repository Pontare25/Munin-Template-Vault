...
1. **Read the capture in full.** If it is a bare URL, fetch the content first if your tool can; otherwise ask the human to paste it.
...
3. **Check what already exists.** For every candidate, search `Atlas/` (start with the folder README indexes) before creating anything. **Update beats duplicate:** if a note on the idea exists, add to it and note the new source; do not create a parallel note.
4. **Create or update atomic notes** in `Atlas/Notes/`, one idea per note, following [[note-schema]] minimums (`type: note`, `up:`, `created-date:`). Start each AI-written note with `%% #AI-Assisted %%`.
5. **Create or update entity pages** in `Atlas/Entities/` for people and organizations that matter to the human (skip names mentioned only in passing).
