...
2. **Check it does not already exist, and check what it contradicts.** Search the target folder's README index and the vault for the title and obvious synonyms. If a close match exists, propose updating that note instead. If the new note would contradict an existing note, create it anyway and link the two via `opposes:`: contradictions are findings to surface, not errors to avoid.
