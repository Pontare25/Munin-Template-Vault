...
1. **Check the GitHub CLI.** Run `gh --version`. If it is missing, tell the user to install it from https://cli.github.com/ and stop. On Windows, after installing you may need to open a fresh terminal, or invoke the full path `C:\Program Files\GitHub CLI\gh.exe`, before `gh` resolves on the command line. Any `fed:*` command also self-checks auth state (via `checkGh` in `fed-lib.mjs`) and prints a plain-language fix rather than a raw error, so a missing or signed-out `gh` is always caught gracefully.
...
- **Single-account or self-test setup.** GitHub will not let an account fork a repo it already owns, so a solo test where the same account owns both the company repo and the personal vault cannot use a real fork. In that case leave `fork` empty in `federation.json`: the contribution flow then opens a same-repo pull request (a branch against `main`) instead of a fork-to-company one. Real employees always work from a separate account and a real fork, which is the intended topology.
